...
### Added an alarm threshold for 205.0F

def get_alarm_threshold() -> float:
    """Fetch the alarm threshold from environment or use default."""
    alarm_threshold = float(os.getenv("SMOKER_ALARM_THRESHOLD", 205.0))
    logger.info(f"Alarm threshold: {alarm_threshold}°F")
...
def process_message(message: str, rolling_window: deque, window_size: int, alarm_threshold: float) -> None:
    """
    ...
    Args:
        ...
        alarm_threshold (float): Temperature threshold for triggering an alarm.
    """
    try:
        ...
        # Check if the temperature exceeds the alarm threshold
        if temperature > alarm_threshold:
            logger.warning(f"ALARM! Temperature {temperature}°F exceeded threshold {alarm_threshold}°F at {timestamp}.")
...
def main() -> None:
    ...
    try:
        for message in consumer:
            ...
            process_message(message_str, rolling_window, window_size, alarm_threshold)
